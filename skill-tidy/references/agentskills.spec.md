...
frontmatter 只描述可被发现和加载阶段使用的元数据。不要把流程步骤、长示例、审查结论、历史说明或 agent 行为约束写入 frontmatter。
...
- 只能作为附加提示，不应成为 skill 能否正确执行的唯一依据。
...
- 使用空格分隔的预批准工具列表。
- 不同实现对它的支持程度可能不同。
- 若目标 skill 需要工具约束，应同时在正文说明真实原因，不要只依赖该字段。
...
可选目录必须服务于目标 skill 的真实执行需求。不要为了目录完整性创建空目录、占位文件或没有被主文档引用的伪资源。
...
8. 检查是否存在编造的运行时、工具、入口、命令、兼容性或外部依赖。
9. 检查未被 `SKILL.md` 链接的附属文件是否仍有必要保留。

若目标环境允许运行 Bun 脚本，可使用 `scripts/validate-skill.ts <目标 skill 目录>` 辅助完成 frontmatter、相对链接、孤立 reference、空壳目录和脚本可执行性检查。脚本只能发现静态结构问题，不能替代用途判断、内容边界和架构合理性评估。
